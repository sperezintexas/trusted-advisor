...
const VALID_EXAM_CODES: ExamCode[] = ['SIE', 'SERIES_7', 'SERIES_57']
...
export default function CoachExamPage() {
    ...
    async (count: number) => {
      ...
      try {
        const res = await fetch(
          `/api/coach/exams/${examCode}/practice-session?count=${count}`
        )
  ...
  const submitExam = useCallback(
    async (saveResults: boolean) => {
      ...
      const res = await fetch(`/api/coach/exams/${examCode}/score`, {
        method: 'POST',
  ...
  const fetchAttemptHistory = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/coach/exams/${examCode}/history?userId=${encodeURIComponent(DEFAULT_USER_ID)}`
      )
      if (!res.ok) return
      const data = await res.json()
  ...
  const handleCheckAnswer = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/coach/exams/${examCode}/check?questionId=${encodeURIComponent(currentQuestionId)}&selectedLetter=${letter}`
      )
